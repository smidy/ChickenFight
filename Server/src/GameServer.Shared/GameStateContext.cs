...
namespace GameServer.Shared
{
    ...
    public class GameStateContext :IDisposable
    {
        ...
        /// Active fights by fightId and a list of playerIds involved
        public Dictionary<string, List<string>> ActiveFights { get; private set; } = new Dictionary<string, List<string>>();
        ...
        public void AddPlayer(string playerId, PlayerMapInfo playerInfo)
        {
            if (playerId != PlayerId)
            {
                ...
                {
                    ActiveFights[playerInfo.FightId].Add(playerId);
                }
            }
        }
        ...
        {
            if (playerId != PlayerId && OtherPlayerInfo.ContainsKey(playerId))
            {
                OtherPlayerInfo[playerId].Position = position;
        ...
        {
            // Generate a fight ID
            ActiveFights.Add(msg.FightId, new List<string>() { msg.Player1Id, msg.Player2Id });
            ...
            if (OtherPlayerInfo.ContainsKey(msg.Player1Id))
            {
                ...
                var updatedInfo = new PlayerMapInfo(currentInfo.Position, msg.FightId);
        ...
        private void OnExtFightEnded(ExtFightEnded msg)
        {
            ...
            ActiveFights.Remove(msg.FightId);
